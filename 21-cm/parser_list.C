...
void parser_list(TString infile="try2" // name of file to parse
    ) { // start main fxn
    ...
    while(getline(ifs,line)) { // loop over lines
        ...
        if (store==kTRUE) { 
            ...
            if (outVect.size()==0) { // if output vector is empty
                ...
                outVect.push_back(new TProfile((TString::Format("%s_%1u", sampleType.Data(), outVect.size())).Data(), sampleType.Data(), nStep, startF+9*nStep, startF+(nStep-10)*stepF)); // make new TProfile of appropriate size

                // fill new TProfile
                for (Int_t i=0; i<nStep; i++) { ss >> mag; if ((i>10)&&(i<nStep-10)) outVect[outVect.size()-1]->Fill(startF+i*stepF,mag,tsys/TMath::Sqrt(0.0078125e6*0.52488));} 
                ...
                if (repeatSource) { // if it already exists
                    ...
                    for (Int_t i=0; i<nStep; i++) { ss >> mag; if ((i>10)&&(i<nStep-10)) outVect[nRepeat]->Fill(startF+i*stepF,mag, tsys/TMath::Sqrt(0.0078125e6*0.52488)); }
                }
                else { // if it doesn't exist, make new TProfile and fill it
                    outVect.push_back(new TProfile((TString::Format("%s_%1u", sampleType.Data(), outVect.size())).Data(), sampleType.Data(), nStep, startF+9*nStep, startF+(nStep-10)*stepF));
    ...
    for (UInt_t i=0; i<outVect.size(); i++) {
        ...
        //histogram(outVect.at(i), "", can, "Frequency (MHz)", "Brightness temperature (K)", TString::Format("histo_%1i", i));
    }
...
Bool_t nameMatch(string line) {
    ...
    return  ((((line.find("G", 0)!=string::npos) && (line.find("G ",0,2)==string::npos)) || ((line.find("Crab",0)!=string::npos) && (line.find("npoint",0)==string::npos)) || ((line.find("PULSAR",0)!=string::npos) && (line.find("npoint",0)==string::npos)) || ((line.find("Sun",0)!=string::npos) && (line.find("npoint",0)==string::npos)) || (line.find("offset",0)!=string::npos) || (line.find("galactic",0)!=string::npos)) && (line.find("file",0)==string::npos) && (line.find("record",0)==string::npos) && (line.find("freq",0)==string::npos));
